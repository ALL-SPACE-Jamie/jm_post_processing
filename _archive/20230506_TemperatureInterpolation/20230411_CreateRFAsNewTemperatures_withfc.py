...
def analyse__RFAparams(filesRFA):
    ...
    for i in range(len(filesRFA)):
        ...
        with open(filesRFA[i], 'r') as file:
            ...
            # meas_array = np.genfromtxt(filesRFA[i], delimiter=',', skip_header=index_start-1)[:,4:][:,::4]
        log_temperature.append(meas_info[19][1])
        log_f_set.append(meas_info[13][1])
        log_beam.append(meas_info[17][1])
        log_board.append(meas_info[21][1])
...
def load__RFA(filePath):
    ...
    with open(filePath, 'r')as file:
        ...
        for row in filecontent:
            meas_info.append(row)
        headerLength = 23
        meas_info = meas_info[0:headerLength]
        meas_array = np.genfromtxt(filePath, delimiter=',', skip_header=headerLength)
        f_measPoints = np.array(meas_info[headerLength-1])[::2].astype(float)
...
for i in range(len(fileNames)):
    ...
    for j in range(len(meas_array)): #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! NOT CORRECT
        beam = meas_info[17][1]
        f_set = meas_info[13][1]
        
        # gain offset
        z = loaded_dict['gain']['beam' + beam[1]]['port' + str(j+1)]['f_set=' + f_set[1:] + 'GHz']
        gain_offset = z[0]*T**2 + z[1]*T + z[2]
        gainT[j,:] = gainT[j,:] - gain_offset
        for zeroCheck in range(len(gainT[j,:])): # set negative values to 0.0
            if gainT[j, zeroCheck] < 0.0:
                gainT[j, zeroCheck] = gainT[j, zeroCheck]*0.0
    ...
    # write new file
    file = open('C:\\codeRun\\_TemperatureInterpolation\\_files\\RFAOUT\\' + fileNames[i][0:-4] + '-' + str(T) + 'C.csv', 'w+', newline ='') 
    with file:     
