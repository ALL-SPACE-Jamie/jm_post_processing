...
# set-up
setUp = {}
setUp['filePath'] = r'C:\Scratch\Rx\Rx_v4_TLM_Thermal_All_forGradGen'
setUp['pathCreate'] = ['figures', 'figuresBAD', 'files', 'pickles', 'overviews']
setUp['filePath_forInterp'] = r'C:\Scratch\Rx\Rx_v4_TLM_Tm_All_Recalc'
setUp['picklePath'] = r'C:\Scratch\Rx\pickles'
setUp['fileOutPath'] = r'C:\Scratch\Rx\files'
freqChoice = '19.70'
...
def find__measFiles(filePath, fileString):
    ...
    for i in range(len(files)):
        if fileString in files[i] and '_'+freqChoice+'_GHz' in files[i]:
...
outFileType = 'OP_2'
find__measFiles(setUp['filePath'], outFileType)
...
for port in range(phaseArray.shape[0]):
    ...
    for beam in calcArrays:
        ...
        for f_c in calcArrays[beam]:
            ...
            plt.grid('on')
            plt.tight_layout()
            plt.savefig('C:\\Scratch\\figures\\Frequency = ' + str(frequency) +
            ...
            if max(y)-min(y):
                plt.savefig('C:\\Scratch\\figuresBAD\\Frequency = ' + str(frequency) +
                            ' GHz, Port = ' + str(port) + ', Beam' + str(beam)[-1] + '_phase.png', dpi=400)
            plt.close('all')
...
for f_c in temperatureGrads[str(0)][beam]:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('C:\\Scratch\\overviews\\' + 'Frequency = ' +
...
find__measFiles(
    r'C:\Scratch\Rx\Rx_v4_TLM_Tm_All_Recalc\2024-02-16_17-10-11_MCR2_Rig2_cal_QR440-0383-00167_19.70_45C\2024-02-17_17-09-42', 'RFA')
for filePath in measFiles:
    print(filePath)
    find__fileDetails(filePath)
    ...
    if meas_params['Temp. [°C]'] == '45':
        ...
        # write new file
        file = open('C:\\Scratch\\files' + '\\' + filePath.split('\\')
                    [-1][0:-4] + '_interp25C.csv', 'w+', newline='')
        with file:
...
for filePath in measFiles:
    ...
    if meas_params['Temp. [°C]'] == '45':
        ...
        # write new file
        file = open('C:\\Scratch\\files' + '\\' + filePath.split('\\')
                    [-1][0:-4] + '_interp65C.csv', 'w+', newline='')
        with file:
            write = csv.writer(file)
