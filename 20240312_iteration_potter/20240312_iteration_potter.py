...
for beam in [1,2]:
    for fig_no in [0,1,2]:
        ...
        for it in [1,2,3,4,5]:
            ...
            stdev = round(np.std(meas_array_gain[:,col]),2)
            pkpk = round(max(meas_array_gain[:,col])-min(meas_array_gain[:,col]),2)
            std_list.append(stdev)
            axs[beam-1, fig_no].plot(meas_array_gain[:,col], alpha=0.4, label=f'iteration {it}, stdev = {stdev}, pkpk = {pkpk}')
        ...
        axs[beam-1, fig_no].set_ylim([-5,10])
