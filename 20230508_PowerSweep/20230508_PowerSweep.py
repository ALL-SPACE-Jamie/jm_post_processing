...
# parmas
filePath = r'C:\codeRun\es2_power_sweeps\es2_power_sweeps\2023-05-04_18-27-58_Minicalrig_powersweep_1_QR00001-es2bu_45C'
...
for file in measFiles:
    ...
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [0, 'V']:
        axs[0,0].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [0, 'H']:
        axs[0,1].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [1, 'V']:
        axs[1,0].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [1, 'H']:
        axs[1,1].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [2, 'V']:
        axs[2,0].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [2, 'H']:
        axs[2,1].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [3, 'V']:
        axs[3,0].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    if S2000_TX_RFIC_CHANNEL_MAP[str(port)] == [3, 'H']:
        axs[3,1].plot(meas_powers, meas_array_gain[:,f_col], label = 'RFIC ' + str(rfic) + ' Port ' + str(port))
    chanCycle = [0,1,2,3]
    linCycle = ['V', 'H']
    for row in range(4):
        for col in range(2):
            axs[row,col].legend(); axs[row,col].set_xlabel('Frequency [GHz]'); axs[row,col].set_ylabel('S$_{21}$ [dB]')
            axs[row,col].set_ylim([-20,20]); axs[row,col].set_xlim([-50, 0.0])
            axs[row,col].set_yticks(np.linspace(-20, 20, num=int(40/5)+1))
            axs[row,col].grid('on')
    ...
    fig.suptitle('Board ' + qr + ', Lens ' + str(lens) + ', Beam' + str(beam) + ', f$_{set}$=' + str(f_c) + ', f$_{meas}$=' + str(f_meas) + ' GHz', fontsize=25)
plt.tight_layout()
plt.show()