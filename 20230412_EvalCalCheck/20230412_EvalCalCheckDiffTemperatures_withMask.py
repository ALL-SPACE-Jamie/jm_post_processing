...
filePath = r'C:\Users\RyanFairclough\Downloads\F-Type_Eval_S_vs_F'
...
#column1 = mask.iloc[:,247].values
#column2 =mask.iloc[:,249].values

#mask.iloc[:,247] = mask.iloc[:255].values
#mask.iloc[:,249] = mask.iloc[:,257].values
#mask.iloc[:,255] = column1
#mask.iloc[:,257] = column2
...
for p in range(2):
    ...
    for l in range(len(f_set_list)):
        ...
        axs[2, 0].text(mean + 0.1, 10.25, str(round(mean, 2)) + ' dB ($\sigma$ = ' + str(round(sigma, 1)) + ')',
                       rotation=90)
        # format
        plt.tight_layout()
