...
for beam in [1,2]:
    ...
    for fig_no in [0]:
        ...
        find_measFiles(file_path, 'RFA', beam, f_set, it=it)
        measFile = measFiles[0]
        load__measFiles(measFile)
        col = np.argmin((meas_frequencies-float(f_set))**2)
        first_file = meas_array_phase[:,col]
        
        for idx in range(len(measFiles)-13):
            ...
            axs[beam-1, fig_no].plot(meas_array_phase[:,col]-first_file, 'o', alpha=1.0)
            # print(np.median(meas_array_phase[:,col]))
        ...
        # axs[beam-1, fig_no].set_ylim([0,100])
        ...
        axs[beam-1, fig_no+1].plot(stdev_array, 'g', linewidth=1.0, label=f'stdev')
        ...
        axs[beam-1, fig_no+2].set_ylim([0,40])
