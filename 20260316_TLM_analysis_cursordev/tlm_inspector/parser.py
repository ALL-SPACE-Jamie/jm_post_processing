# -*- coding: utf-8 -*-
"""
TLM Inspector - File parsing and data discovery.
...
def build_full_results(df_index: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    -------
    pd.DataFrame
        One row per (file, port) with metadata columns plus ``gain``
        and ``phase`` at the setpoint frequency.
    """
    meta_cols = [
...
def discover_files(root_path: str) -> pd.DataFrame:
    """
    ...
    records = []
    for csv_path in root.rglob("iteration_*/*.csv"):
        try:
