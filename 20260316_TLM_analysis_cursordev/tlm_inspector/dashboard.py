# -*- coding: utf-8 -*-
"""
TLM Inspector - Dash layout and callbacks.
...
# Store last-generated stats parameters for histogram callback
_stats_params: dict = {}
...
def _stats_tab() -> html.Div:
    return html.Div([
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P(
                "Enter a port number to see the distribution of values "
                "across all TLM files at the selected frequency.",
                style={"color": COLORS["text_muted"], "fontSize": "13px",
                        "marginBottom": "12px"},
            ),
            dbc.Row([
                dbc.Col([
                    _label("Port Number (1\u2013456)"),
                    dbc.Input(
                        id="stats-hist-port", type="number",
                        placeholder="e.g. 1",
                        className="bg-dark text-light border-secondary",
                        min=1, max=456, step=1,
                    ),
                ], width=2),
                dbc.Col([
                    _label("\u00A0"),
...
def _docs_tab() -> html.Div:
    ...
    return html.Div([
        html.Div(style=CARD_STYLE, children=[
            ...
            html.Pre(
                "app.py                  \u2190 Entry point (Dash server)\n"
                "tlm_inspector/\n"
                "  \u251C\u2500 config.py           \u2190 Theme, constants, version\n"
                "  \u251C\u2500 parser.py           \u2190 File discovery, CSV parsing, geometry\n"
                "  \u251C\u2500 plots.py            \u2190 Plotly figure generation\n"
                "  \u251C\u2500 dashboard.py        \u2190 Dash layout and callbacks\n"
                "  \u2514\u2500 geometry/           \u2190 Bundled array geometry data\n"
                "assets/\n"
            ...
            html.Ol([
                html.Li("User provides a root TLM directory on the Data tab."),
                html.Li(
                    "parser.discover_files() recursively scans for CSV files "
                    "and builds a metadata index DataFrame."
                ),
                html.Li(
                    "The index is stored in dcc.Store (client-side JSON) and "
                    "shared across all tabs."
                ),
                html.Li(
                    "Plotting callbacks filter the index, load file data "
                    "(via parser.get_file_data with caching), compute "
                    "statistics, and generate Plotly figures."
                ),
                html.Li(
                    "Figures can be displayed inline or exported as PNG images "
...
def register_callbacks(app: Dash) -> None:
    ...
    @app.callback(
        ...
        Output("load-progress", "style"),
        Input("load-data-btn", "n_clicks"),
        State("data-path-input", "value"),
        prevent_initial_call=True,
    )
    def load_data(n_clicks, path):
        hide_bar = {"marginTop": "12px", "height": "6px", "display": "none"}
        if not path:
            return (
                no_update,
    ...
    @app.callback(
        ...
        Input("build-full-btn", "n_clicks"),
        State("data-store", "data"),
        prevent_initial_call=True,
    )
    def build_full_results(n_clicks, store_data):
        if not store_data:
            return no_update, _status_badge("Load data first", "warning")

        try:
            df_index = pd.read_json(store_data, orient="split")
            df_full = parser.build_full_results(df_index)
            if df_full.empty:
    ...
    def generate_port_histogram(n_clicks, port_number):
        if not _stats_params or port_number is None:
            return _placeholder(
                "Enter a port number and generate the patch map first."
            )

        port_number = int(port_number)
        if port_number < 1 or port_number > 456:
            return _placeholder("Port number must be between 1 and 456.")

        try:
            fig = plots.create_port_histogram(
                _stats_params["files_b1"],
                _stats_params["files_b2"],
                _stats_params["depvar"],
                _stats_params["frequency"],
                port_number,
            )
        except Exception as exc:
            return dbc.Alert(
