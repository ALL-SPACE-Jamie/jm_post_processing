...
filePath = r'C:\Users\JamieMitchell\OneDrive - ALL.SPACE\S-Type\Rx_TLM\ES2c-Laser_Cut\TLM_Calibration_RFA_Files\Rx_Batch_3\Post-processed_RFA_Temp_45deg\Post_Step1_Fixed_QR'
normVal = 3.00
...
for beamChoice in range(2):
    ...
    def find__RFAfiles(path, f_set, beam):
        ...
        for i in range(len(files)):
            # if 'RFA' in files[i] and 'L6-BS2' not in files[i] and 'both_' + str(f_set) + '_GHz' in files[i] and 'Beam'+str(beam) in files[i]:
    ...
    for f_set in f_set_Log:
        ...
        # for j in range(2):
            ...
            savePath =  'C:\\Scratch\\files_rx-s3'
            if not os.path.exists(savePath):
                os.makedirs(savePath) 
            # write new file
